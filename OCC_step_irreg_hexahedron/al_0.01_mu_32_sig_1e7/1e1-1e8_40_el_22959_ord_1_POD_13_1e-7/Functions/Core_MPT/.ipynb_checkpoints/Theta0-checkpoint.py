...
def Theta0(fes, Order, alpha, mu_inv, inout, e, Tolerance, Maxsteps, epsi, simnumber, Solver, Additional_Int_Order, use_longdouble=True):
    ...
    inverse = CGSolver(a.mat, c.mat, tol=Tolerance, maxiter=Maxsteps, printrates=False)
    ...
    if inverse.iterations == inverse.maxiter:
        warnings.warn(f'Solver did not converge within {inverse.maxiter} iterations. Solution may be inaccurate.')

    # # Saving convergence history:
    # err = np.asarray(inverse.errors)
    # np.savetxt(f'Theta0_convergence_rates_e{simnumber}.csv', err, delimiter=',')
