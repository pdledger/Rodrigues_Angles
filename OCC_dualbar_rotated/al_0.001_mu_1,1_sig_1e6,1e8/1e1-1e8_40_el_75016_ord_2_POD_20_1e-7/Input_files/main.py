...
def main(h='coarse', order=2, curve_degree=5, start_stop=(), alpha='', geometry='default', frequency_array='default', use_OCC=False,
         use_POD=False, use_parallel=True, use_iterative_POD=False, cpus='default', N_POD_points='default', POD_array='default'):
    """
    ...
    if POD_array != 'default':
    	PODArray = POD_array # Overwriting logspaced POD array with custom array from input to function
...
if __name__ == '__main__':
    ...
    # filelist = ['Tetra_homogeneous.geo', 'TwoTetra_homogeneous.geo', 'ThreeTetra_homogeneous.geo', 'FourTetra_homogeneous.geo']#, 'OCC_cube_prism.py', 'OCC_dualbar.py', 'OCC_sphere_prism_32.py']
    # ord = [3,3,3,3]
    
    # filelist = ['Tetra_homogeneous.geo']
    # filelist = ['TwoTetra.geo'] * 5
    # ord = [0,1,2,3,4]
    # count = 0
    # for file, p in zip(filelist, ord):
        
    #     print('\033[96m'+ f'Solving for p={p}' + '\033[00m')
        
    #     output1 = main(order=p, geometry=file, use_OCC=False, use_parallel=True, cpus=1, use_POD=True, curve_degree=1, start_stop=(1,8,40))

    #     # Z_upper_bound = np.genfromtxt('Z_upper_bound.csv')
    #     # Z_tilde_upper_bound = np.genfromtxt('Z_tilde_upper_bound.csv')

    #     # Z_Prisms_not_weighted = []
    #     # for k, omega in enumerate(output1['FrequencyArray']):
    #     #     R = output1['TensorArray'][k,:].reshape(3,3).real
    #     #     I = output1['TensorArray'][k,:].reshape(3,3).imag
    #     #     Z_Prisms_not_weighted += [np.linalg.norm(R@I - I@R)]
    #     # plt.figure()
    #     # plt.loglog(output1['FrequencyArray'], Z_Prisms_not_weighted, label=r'$||\tilde{Z}||_F$')
    #     # plt.loglog(output1['PODFrequencyArray'], Z_tilde_upper_bound, label=r'$\Delta(||\tilde{Z}||)$')
        
    #     # plt.xlabel(r'$\omega$, [rad/s]')
    #     # plt.ylabel(r'$||\tilde{Z}||_{F}$')
    #     # plt.legend()
    ...
    #     # Z_Prisms_not_weighted = []
    #     # for k, omega in enumerate(output1['FrequencyArray']):
    #     #     R = output1['TensorArray'][k,:].reshape(3,3).real - output1['N0'].reshape(3,3)
    #     #     I = output1['TensorArray'][k,:].reshape(3,3).imag
    #     #     Z_Prisms_not_weighted += [np.linalg.norm(R@I - I@R)]
    #     # plt.figure()
    #     # plt.loglog(output1['FrequencyArray'], Z_Prisms_not_weighted, label=r'$||Z_{comput}||_F$')
    #     # plt.loglog(output1['PODFrequencyArray'], Z_upper_bound, label=r'$\Delta(||Z||)$')
    ...
    #     sum_reg_terms = np.genfromtxt('sum_reg_terms_real.csv', dtype=complex)
    #     sum_reg_terms = np.genfromtxt('sum_reg_terms_imag.csv', dtype=complex)
        
    #     plt.figure(999)
    #     label_list = [12, 13, 21, 23, 31, 32]
    #     cols = ['tab:blue', 'tab:green', 'tab:red', 'tab:pink', 'tab:orange', 'tab:brown']
    #     marks = ['x', '+', '*', '^', '1']
    #     for ind, coeff in enumerate([0]):
    #         plt.loglog(output1['PODFrequencyArray'], np.abs(output1['PODTensorArray'][:, coeff].real), label=f'$|\mathcal{{R}}_{{{label_list[ind]}}}|, p={p}$', color=cols[count], marker=marks[count])
    #         plt.loglog(output1['PODFrequencyArray'], np.abs(sum_reg_terms[:, coeff].real), label=f'$|\Delta(\mathcal{{R}})_{{{label_list[ind]}}}|, p={p}$', linestyle='--', color=cols[count], marker=marks[count])
    #     plt.legend()
    #     plt.xlabel(r'$\omega$, [rad/s]')
    #     plt.ylabel(r'$|\mathcal{R}|_{12}$')

    #     plt.figure(1000)
    #     plt.semilogx(output1['FrequencyArray'], output1['TensorArray'][:,0].real, label=f'p={p}')
    #     plt.legend()
    #     plt.xlabel(r'$\omega$, [rad/s]')
    #     plt.ylabel(r'$\mathcal{R}_{11}$')
        
        
        
    #     plt.figure(1001)
    #     plt.semilogx(output1['FrequencyArray'], output1['TensorArray'][:,1].real, label=f'p={p}')
    #     plt.legend()
    #     plt.xlabel(r'$\omega$, [rad/s]')
    #     plt.ylabel(r'$\mathcal{R}_{12}$')

    #     # plt.figure()
    #     # col = ['k', 'tab:blue', 'tab:orange', 'tab:red', 'k', 'tab:green', 'tab:cyan', 'tab:pink', 'k']
    #     # for k in [1,2,5]:
    #     #     # plt.semilogx(output2['FrequencyArray'], output2['TensorArray'][:,k].real, color=col[k], linestyle='--', label=f'No Prisms: ind={k}')
    #     #     plt.semilogx(output1['FrequencyArray'], output1['TensorArray'][:,k].real, color=col[k], linestyle='-', label=f'With Term3: ind={k}')
    #     #     plt.legend(prop={'size':8})
    #     #     plt.xlabel(r'$\omega$, [rad/s]')
    #     #     plt.ylabel(r'Real off diagonal terms')

    #     # plt.figure()
    #     # col = ['k', 'tab:blue', 'tab:orange', 'tab:red', 'k', 'tab:green', 'tab:cyan', 'tab:pink', 'k']
    #     # for k in [1, 2, 5]:
    #     #     plt.semilogx(output1['FrequencyArray'], output1['TensorArray'][:, k].imag, color=col[k], linestyle='-',
    #     #                  label=f'With Prisms: ind={k}')
    #     #     plt.legend(prop={'size': 8})
    #     #     plt.ylabel(r'Imag off diagonal terms')
    #     #     plt.xlabel(r'$\omega$, [rad/s]')
    ...
    #     #     R = output1['TensorArray'][k,:].reshape(3,3).real
    #     #     I = output1['TensorArray'][k,:].reshape(3,3).imag
    #     #     R_offdiag += [np.linalg.norm(R[mask_ind[0], mask_ind[0]])]
    #     #     I_offdiag += [np.linalg.norm(I[mask_ind[0], mask_ind[0]])]

    #     # plt.loglog(output1['FrequencyArray'], R_offdiag, label='real')
    #     # plt.loglog(output1['FrequencyArray'], I_offdiag, label='imag')
    #     # plt.legend()

    #     #plt.show()

    #     save_all_figures(f'OutputFigures_25Oct/{file}_p={p}', format='pdf')
    #     count += 1
    #     del output1
    #     #plt.close('all')
        
    #     for i in plt.get_fignums():
    #         if i != 999 and i != 1000 and i != 1001:
    #             plt.close(i)


    filelist = ['OCC_dualbar_rotated.py'] * 5
    ord = [0,1,2,3]
    cpu_list = [4,4,4,2]
    count = 0
    for file, p, c in zip(filelist, ord, cpu_list):
        
        print('\033[96m'+ f'Solving for p={p}' + '\033[00m')
        
        custom_PODArray = np.sort(np.concatenate((np.logspace(1,8,17), np.asarray([1.5*1e7, 3*1e7, 5*1e7]))))
        
        output1 = main(order=p, geometry=file, use_OCC=True, use_parallel=True, cpus=c, use_POD=True, curve_degree=1, start_stop=(1,8,40), POD_array=custom_PODArray)
        
        
        sum_reg_terms_real = np.genfromtxt('sum_reg_terms_real.csv', dtype=complex)
        sum_reg_terms_imag = np.genfromtxt('sum_reg_terms_imag.csv', dtype=complex)
        
        plt.figure(999)
        label_list = [12, 13, 21, 23, 31, 32]
        cols = ['tab:blue', 'tab:green', 'tab:red', 'tab:pink', 'tab:orange', 'tab:brown']
        marks = ['x', '+', '*', '^', '1']
        for ind, coeff in enumerate([1]):
            plt.loglog(output1['FrequencyArray'], np.abs(output1['TensorArray'][:, coeff].real), label=fr'$|(\tilde{{ \mathcal{{R}} }}^{{hp}})_{{{label_list[ind-1]}}}|, p={p}$', color=cols[count])#, marker=marks[count])
            # plt.loglog(output1['FrequencyArray'], np.abs(sum_reg_terms_real[:, coeff].real), label=f'$|\Delta(\mathcal{{R}})_{{{label_list[ind]}}}|, p={p}$', linestyle='--', color=cols[count], marker=marks[count])
            plt.legend()
            plt.xlabel(r'$\omega$, [rad/s]')
            plt.ylabel(rf'$|(\tilde{{ \mathcal{{R}} }}^{{hp}})_{{{label_list[ind-1]}}}|$')
        ...
        marks = ['x', '+', '*', '^', '1']
        for ind, coeff in enumerate([1]):
            plt.loglog(output1['FrequencyArray'], np.abs(output1['TensorArray'][:, coeff].imag), label=f'$|(\mathcal{{I}}^{{hp}})_{{{label_list[ind]}}}|, p={p}$', color=cols[count])#, marker=marks[count])
            # plt.loglog(output1['FrequencyArray'], np.abs(sum_reg_terms_imag[:, coeff].real), label=f'$|\Delta(\mathcal{{I}})_{{{label_list[ind]}}}|, p={p}$', linestyle='--', color=cols[count], marker=marks[count])
            plt.legend()
            plt.xlabel(r'$\omega$, [rad/s]')
            plt.ylabel(rf'$(\mathcal{{I}}^{{hp}})_{{{label_list[ind]}}}$')
        
        plt.figure(1001)
        label_list = [12, 13, 21, 23, 31, 32]
        cols = ['tab:blue', 'tab:green', 'tab:red', 'tab:pink', 'tab:orange', 'tab:brown']
        marks = ['x', '+', '*', '^', '1']
        for ind, coeff in enumerate([2]):
            plt.loglog(output1['FrequencyArray'], np.abs(output1['TensorArray'][:, coeff].real), label=fr'$|(\tilde{{ \mathcal{{R}} }}^{{hp}})_{{{label_list[ind-1]}}}|, p={p}$', color=cols[count])#, marker=marks[count])
            # plt.loglog(output1['FrequencyArray'], np.abs(sum_reg_terms_real[:, coeff].real), label=f'$|\Delta(\mathcal{{R}})_{{{label_list[ind-1]}}}|, p={p}$', linestyle='--', color=cols[count], marker=marks[count])
            plt.legend()
            plt.xlabel(r'$\omega$, [rad/s]')
            plt.ylabel(rf'$|(\tilde{{ \mathcal{{R}} }}^{{hp}})_{{{label_list[ind-1]}}}|$')
        ...
        marks = ['x', '+', '*', '^', '1']
        for ind, coeff in enumerate([2]):
            plt.loglog(output1['FrequencyArray'], np.abs(output1['TensorArray'][:, coeff].imag), label=f'$|(\mathcal{{I}}^{{hp}})_{{{label_list[ind-1]}}}|, p={p}$', color=cols[count])#, marker=marks[count])
            # plt.loglog(output1['FrequencyArray'], np.abs(sum_reg_terms_imag[:, coeff].real), label=f'$\Delta(|(\mathcal{{I}})_{{{label_list[ind-1]}}}|), p={p}$', linestyle='--', color=cols[count], marker=marks[count])
            plt.legend()
            plt.xlabel(r'$\omega$, [rad/s]')
            plt.ylabel(fr'$|(\mathcal{{I}}^{{hp}})_{{{label_list[ind-1]}}}|$')
        ...
        for k, omega in enumerate(output1['FrequencyArray']):
            ...
            Z_Prisms_not_weighted += [np.linalg.norm(R@I - I@R)]
        plt.figure()
        plt.loglog(output1['FrequencyArray'], Z_Prisms_not_weighted, label=r'$\Vert\tilde{\mathcal{Z}}^{hp}\Vert_F$')
        plt.loglog(output1['FrequencyArray'], Z_tilde_upper_bound, label=r'$\Delta(\Vert \tilde{Z}\Vert)$')
        
        plt.xlabel(r'$\omega$, [rad/s]')
        plt.ylabel(r'$\Vert\tilde{\mathcal{Z}}\Vert_{F}$')
        plt.legend()
        ...
        for k, omega in enumerate(output1['FrequencyArray']):
            ...
            Z_Prisms_not_weighted += [np.linalg.norm(R@I - I@R)]
        plt.figure()
        plt.loglog(output1['FrequencyArray'], Z_Prisms_not_weighted, label=r'$\Vert {\mathcal{Z}}^{hp}\Vert_F$')
        plt.loglog(output1['FrequencyArray'], Z_upper_bound, label=r'$\Delta(\Vert {\mathcal{Z}}\Vert_F)$')

        plt.xlabel(r'$\omega$, [rad/s]')
        plt.ylabel(r'$\Vert \mathcal{Z} \Vert_{F}$')
        plt.legend()
        
        

        save_all_figures(f'OutputFigures_17Nov/{file}_p={p}', format='pdf')
        count += 1
        del output1#, sum_reg_terms_real, sum_reg_terms_imag
        #plt.close('all')
        
        for i in plt.get_fignums():
            if i != 999 and i != 1000 and i != 1001 and i != 1002:
                plt.close(i)
